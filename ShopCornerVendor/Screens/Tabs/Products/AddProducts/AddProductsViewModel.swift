...
class AddProductsViewModel: ObservableObject {
    ...
    var products:[AddProductsModel.ProductData] {
        product.products
    }
    ...
    func addProduct(product: AddProductsModel.ProductData) {
...
//    func uploadAllProducts(completion: @escaping (Result<Void, Error>) -> Void) {
//        uploadNextProduct(at: 0, completion: completion)
//    }
    ...
    private func uploadNextProduct(at index: Int, completion: @escaping (Error?) -> Void) {
        ...
        guard let image = product.productImage else {
            print("Product \(product.productName) has no image. Skipping.")
...
//    
    func uploadProduct(_ product: AddProductsModel.ProductData, url: String, completion: @escaping (Error?) -> Void) {
        let vendId = AuthService.shared.getVendor().id
        let request = product.getRequest(url: url, venderID: vendId)
        repository.addProduct(request) { _, error in
            completion(error)
        }
